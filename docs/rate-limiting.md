...
Rate limiting caps **how fast a queue dispatches tasks** — "run `email:send` at
most 50 per second" — so a queue can't overwhelm a fragile downstream (a
...
restarts) — no redeploy to change one.
...
`ListRateLimits` — the same calls the CLI and dashboard make, usable from any
...
- **Changes take effect immediately** — no worker or server restart.
...
A throttled queue increments the `conveyor_ratelimit_throttled` counter (labeled
by queue) each time it defers dispatch on an exhausted limit — watch it to see

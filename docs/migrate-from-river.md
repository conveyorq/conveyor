...
| Dead-letter | OSS: exhausted jobs go to a terminal `discarded` state | OSS: exhausted jobs are archived (inspectable) — parity                                                  |
| Form factor | Library only                                           | Library (embedded) **and** standalone server with a language-neutral protocol                            |
| Web UI      | riverui                                                | None yet                                                                                                 |
...
| `Tags` / `Metadata` | —                                                | attaching arbitrary metadata at enqueue is not exposed in the SDK yet                                        |
...
- **Type-safe generic job args** — Conveyor uses opaque payloads + `Bind`.
- **riverui**, and a more mature, commercially backed project with wider
  adoption. For production, weigh that seriously: River is further along.

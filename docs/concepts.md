...
the handler) and a **payload** (the bytes the handler needs — JSON, protobuf, or
raw). Tasks may be enqueued to run now, after a delay, at a scheduled time, or on
...
the worker's declared concurrency** — it will never have more than that many
...
- **Failures and retries.** A handler that returns an error is retried with
  ...
  that panics is recovered and treated as a retryable failure — one bad task

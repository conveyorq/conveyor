...
A `nil` encryptor is ignored — encryption stays off — so you can pass an
...
- **Payloads are encrypted.** Task metadata, type, queue, and timing are not —
  the server needs them to route and schedule. Keep secrets in the payload.
- **The dashboard shows ciphertext** for encrypted tasks. The server cannot
  decrypt for display because it has no key — that is the point.
...
- [`encryption`](../encryption) package — the `Encryptor` seam and the built-in

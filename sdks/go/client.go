...
func (c *Client) Enqueue(ctx context.Context, task *Task, opts ...EnqueueOption) (*TaskInfo, error) {
	settings, uniqueKey, err := resolveEnqueueOptions(task, opts...)
	if err != nil {
		return nil, err
	}
	...
	core := func(ctx context.Context, task *Task) (*TaskInfo, error) {
		request, err := c.buildEnqueueRequest(ctx, task, settings, uniqueKey)
		if err != nil {
			return nil, err
		}
...
// resolveEnqueueOptions validates a task and resolves its per-call options,
// returning the resolved settings and the effective unique key. The unique key
// is derived over the plaintext payload, before any encryption, so identical
// work still collides under Unique while the server only ever sees ciphertext.
func resolveEnqueueOptions(task *Task, opts ...EnqueueOption) (*enqueueOptions, string, error) {
	if task == nil {
	...
	return settings, uniqueKey, nil
}

// buildEnqueueRequest builds the wire request for a task from its resolved
// options and unique key, sealing the payload when encryption is configured. It
// is the single source of truth shared by Enqueue and EnqueueTx, so both produce
// an identical wire request for the same inputs.
func (c *Client) buildEnqueueRequest(ctx context.Context, task *Task, settings *enqueueOptions, uniqueKey string) (*conveyorv1.EnqueueRequest, error) {
	payload := task.payload
	metadata := task.metadata

	// Seal the payload after the unique key was derived over plaintext, so the
	// server only ever sees ciphertext. An empty payload stays empty.

...
> **Status (2026-06-12):** Phases 0–2, 3a, and 3b are ✅ COMPLETE. Next up: **Phase 4**. One open caveat: the Phase 2 5k dispatches/s gate is skip-blocked on the upstream GoAkt TellGrain local fast path (see `TestQueueGrainDispatchThroughput`); bump the GoAkt pin and unskip when it ships. Phase 3b notes: the CLI ships `enqueue` and `tasks get` only — every other §11 command needs the AdminService, which lands in Phase 4; `Unique`/`UniqueKey` pass through on the wire but enforcement is Phase 4; server-side session drain on shutdown is Phase 4 (a stopping node currently waits out live worker streams).
